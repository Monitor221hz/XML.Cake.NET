...
public class XPathLookup
{
    ...
    public bool TryLookupPath(XNode node, out string? path) =>
        nodePaths.TryGetValue(node, out path);
    ...
    public bool AddTrackedNode(XmlReader reader, XPathTracker tracker) =>
        AddTrackedNode(reader, tracker.GetCurrentPath(reader));
    ...
    public bool AddTrackedNode(XNode node, XPathTracker tracker) =>
        AddTrackedNode(node, tracker.GetCurrentPathByElement(node));
}
