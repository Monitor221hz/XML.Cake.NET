...
public class XPathTracker
{
    ...
    public string GetCurrentPath(XmlReader reader) =>
        string.Join("/", trackedPath.SkipLast(maxDepth - reader.Depth));

    public string GetCurrentPathByElement(XNode node) =>
        string.Join("/", trackedPath.SkipLast(Math.Abs(maxDepth - GetElementDepth(node))));
    ...
    public static bool IsContentNode(XmlNodeType nodeType) =>
        (XmlNodeType.EndElement != nodeType && XmlNodeType.Comment != nodeType);
