...
public class XMapElement : XElement, IXMap
{
    ...
    public Dictionary<string, XElement> mappedElements { get; private set; } =
    ...
    public XMapElement(XElement document, Func<XElement, string> generateKey)
        : base(document)
    {
    ...
    public XMapElement(XElement document)
        : base(document)
    {
    ...
    public bool TryLookup(string path, out XElement element) =>
        mappedElements.TryGetValue(path, out element!);
    ...
    /// <returns></returns>
    public XElement NavigateTo(
        string path,
        XElement workingRoot,
        Func<XElement, string> generateKey
    )
    {
        XElement element;

        if (mappedElements.TryGetValue(path, out element!))
            return element;
    ...
    /// <param name="workingRoot"></param>
    /// <returns></returns>
    public XElement NavigateTo(string path, XElement workingRoot) =>
        NavigateTo(path, workingRoot, GenerateKey);
    ...
    public void MapLayer(bool ignoreParents, params int[] pathIndices)
    {
        ...
        for (int d = 0; d < pathIndices.Length; d++)
        {
            ...
            if (!ignoreParents)
                MapChildElement(parentPathBuilder.ToString(), pathElement, 0);
    ...
    public void MapSlice(
        string rootPath,
        int depth,
        bool useBlankPath,
        Action<string, XElement> mapFunc
    ) //vertical mapping
    {
        if (depth < 1)
            return;
    ...
    public void MapSlice(string rootPath, bool useBlankPath)
    {
        ...
        XElement workingRoot = !String.IsNullOrEmpty(rootPath)
            ? thisKey == rootPath
                ? this
                : NavigateTo(rootPath)
            : this;
    ...
    public class XNodeMarker
    {
        ...
        public XNode GetMarkedNode() =>
            refNode == null
                ? parent.Elements().First()
                : (forward ? refNode.PreviousNode! : refNode.NextNode!);
    }
    ...
    public string InsertElement(string path, XElement newElement, bool remapParent)
    {
        ...
        lock (targetElement.Parent)
            lock (targetElement)
            {
                ...
                newElement = (XElement)(marker.GetMarkedNode());
                if (remapParent)
                    MapSlice(targetElement.Parent!, false, MapResetDuplicates);
                newPath = GetPath(
                    parentPath,
                    newElement,
                    newElement.ElementsBeforeSelf().Count() - 1
                );
            }
        return newPath;
    }
    ...
    public string AppendElement(string path, XElement element)
    {
        ...
        string newPath = string.Empty;
        //
        if (!TryLookup(path, out parentElement))
            return newPath;
        lock (parentElement)
        {
            parentElement.Add(element);
            element = (XElement)parentElement.LastNode!;
            newPath = MapChildElement(
                path,
                element,
                element.ElementsBeforeSelf().Count(),
                MapResetDuplicates
            );
        }
        return newPath;
    ...
    private string GetDefaultPath(string path, string defaultValue, int elementIndex)
    {
        ...
        if (IsOpenPath)
            defaultBuilder.Append(XPATH_DIVIDER);
    ...
    private string GetUniquePath(string path, string key)
    {
        ...
        if (IsOpenPath)
            uniqueBuilder.Append(XPATH_DIVIDER);
    ...
    private XElement FindChildByKey(XElement parent, string key, Func<XElement, string> generateKey)
    {
        ...
        return parent
            .Elements()
            .Where(e => generateKey(e) == key)
            .FirstOrDefault(new XElement("null"));
    }

    private XElement FindChildByKey(XElement parent, string key) =>
        FindChildByKey(parent, key, GenerateKey);
    ...
    )
    {
        ...
        key =
            (key == defaultValue)
                ? GetDefaultPath(path, defaultValue, elementIndex)
                : GetUniquePath(path, key);
        return key;
    }
    ...
    private string MapChildElement(
        string path,
        XElement element,
        int elementIndex,
        Action<string, XElement> CustomMapElementToPath
    )
    {
